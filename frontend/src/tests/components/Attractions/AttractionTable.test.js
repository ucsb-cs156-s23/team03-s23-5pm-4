...
describe("AttractionTable tests", () => {
  ...
  test("renders without crashing for empty table with user not logged in", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <AttractionTable attractions={[]} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>
  ...
  test("Has the expected column headers and content for adminUser", () => {
    ...
    const { getByText, getByTestId } = render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <AttractionTable attractions={attractionFixtures.threeAttractions} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>
  ...
  test("Edit button navigates to the edit page for admin user", async () => {
    ...
    await waitFor(() => { expect(getByTestId(`AttractionTable-cell-row-0-col-id`)).toHaveTextContent("2"); });

    const editButton = getByTestId(`AttractionTable-cell-row-0-col-Edit-button`);
    expect(editButton).toBeInTheDocument();
  ...
  test("Details button navigates to the details page for admin user", async () => {
    ...
    const detailsButton = getByTestId(`AttractionTable-cell-row-0-col-Details-button`);
